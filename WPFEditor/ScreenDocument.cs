...
namespace MegaMan.WPFEditor
{
    public class ScreenDocument
    {
        ...
        public void DrawOn(DrawingContext context)
        {
            screen.Draw(context, 0, 0, screen.PixelWidth, screen.PixelHeight);
        }

        public void DrawEntities(DrawingContext context)
        {
            for (var i = 0; i < screen.EnemyInfo.Count; i++)
            {
                ...
                if (sprite != null)
                {
                    sprite.Draw(context, (float)info.screenX, (float)info.screenY);

                    if (selectedEntityIndex == i)
                    {
                        context.DrawRectangle(Pens.LimeGreen, (float)info.screenX - sprite.HotSpot.X, (float)info.screenY - sprite.HotSpot.Y, sprite.Width, sprite.Height);
                    }
                }
                else
                {
                    context.DrawImage(Properties.Resources.nosprite, (float)info.screenX - 8, (float)info.screenY - 8);

                    if (selectedEntityIndex == i)
                    {
                        context.DrawRectangle(Pens.LimeGreen, (float)info.screenX - 8, (float)info.screenY - 8, 16, 16);
                    }
                }
            }
        }
